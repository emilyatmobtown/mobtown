...
?>
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
                <?php if(get_post_meta(get_the_ID(), "eltd_hide-featured-image", true) != "yes" && !is_singular( array( 'post' ) ) ) {
                    if ( has_post_thumbnail() ) { ?>
                        <div class="post_image">
                            <?php the_post_thumbnail('full'); ?>
                        </div>
...
<!--
                            <?php if( is_singular( array( 'post' ) ) ) { 
	                            if ( has_post_thumbnail() ) { ?>
			                        <div class="post_image">
			                            <?php the_post_thumbnail( 'mob-blog-header-image' ); ?>
			                        </div>
							<?php } } ?>
-->
							<?php
								if( is_singular( array( 'post' ) ) && has_post_thumbnail() ) {
									$paragraphAfter[2] = '<div class="post_image">' . get_the_post_thumbnail( $post->ID, 'full' ) . '</div>';
								}
								
								$content_array = explode( '</p>', apply_filters('the_content', get_the_content() ) );
								$newcontent = '';
                                ...
                                echo $content;
							?>
							<div class="mob-post-extras">
										
								<?php if(isset($eltd_options['blog_single_show_social_share']) && $eltd_options['blog_single_show_social_share'] == "yes" && $blog_social_share_type == "list") { ?>
									<div class="mob-social-share">
										<h2 class="mob-widget-title mob-share-title">Share</h2>
										<?php echo do_shortcode('[no_social_share_list]'); // XSS OK ?>
									</div>
								<?php } ?>		
			                 
								<?php if( is_singular( array( 'post' ) ) ) { ?>
								    <div class="single_tags mob-categories clearfix">
							            <h2 class="mob-widget-title mob-share-title"><?php _e('Learn More', 'eltd'); ?></h2>
							            <?php the_category('', '', ''); ?>
								    </div>
			                    <?php } ?>
		
								<?php if( is_singular( array( 'post' ) ) ) { ?>
			                     <div class="mob-related-posts">
				                     <h2 class="mob-widget-title mob-share-title"><?php _e('You Might Also Like', 'eltd'); ?></h2>
				                    <?php echo do_shortcode( '[pt_view id="9a16ccdast" cat="GET_CURRENT"]' ); ?>
			                    </div>
			                    <?php } ?>
							</div>
                   </div>
