<?php 
/** 	
 *	Summary. Template for title area
 *	
 * 	Description. Outputs layout and content for title area based on post type.
 *
 *	@package mobtown
 *	@since 1.0.0 
 */	
...
if(get_post_meta($id, "eltd_fixed-title-image", true) != ""){
	...
	if( is_singular( array( 'record' ) ) ) {
		$fixed_title_image = 'yes'; // Add record cover for header image -ELS
	} elseif( is_singular( array( 'post' ) ) ) {
		$fixed_title_image = 'yes'; // Add header image from "Learn" Page -ELS
	} else {
...
if(get_post_meta($id, "eltd_title-image", true) != ""){
	...
	if( is_singular( array( 'record' ) ) ) {
		$title_image = get_the_post_thumbnail_url( $id, 'mob-header-image' ); // Add record cover for header image -ELS
	} elseif( is_singular( array( 'post' ) ) ) {
		$parent_page = get_page_by_path( 'learn' );
		$title_image = get_post_meta( $parent_page->ID, "eltd_title-image", true ); // Add header image from "Learn" Page -ELS
	} else {
...
// Add title text for single post -ELS
if( is_singular( array( 'post' ) ) ) {
    ...
    $title = __('You Searched For', 'eltd').' '.get_search_query();
}elseif (is_404()){
...
if($is_title_area_visible) { ?>
	<div class="title_outer <?php echo esc_attr($animate_title_class.$title_text_shadow); if($responsive_title_image == 'yes' && $show_title_image == true && $title_image !== ''){ echo ' with_image'; }?>" <?php echo esc_attr($animation); ?> <?php echo 'data-height="'.esc_attr($title_height).'"'; if($title_height != '' && $animate_title_area == 'area_top_bottom'){ echo ' style="opacity:0; height:0px;"'; } ?>>
		<div class="title <?php echo is_singular( array( 'post', 'record' ) ) ? 'has_fixed_background' : ''; ?>  <?php eltd_title_classes(); ?>" style="<?php if($responsive_title_image == 'no' && $title_image != "" && $show_title_image == true){ if($title_image_width != ''){ echo 'background-size:'.esc_attr($title_image_width).'px auto;'; } echo 'background-image:url('.esc_url($title_image).');';  } if($title_height != ''){ echo 'height:'.esc_attr($title_height).'px;'; } if($title_background_color != ''){ echo 'background-color:'.esc_attr($title_background_color).';'; } ?>">
